-- ============================================================================
-- Scout Dashboard Compatibility Package - Fixed Migration
...
-- Create data catalog table if not exists with proper unique constraint
    ...
    SELECT
        CASE
            ...
            WHEN p_filters ? 'date_range' THEN
                format('transaction_date >= %L', p_filters->>'date_range')
            ELSE '1=1'
        END;
$$;
...
AS $$
    WITH regional_performance AS (
        ...
        FROM scout.fact_transactions ft
        LEFT JOIN scout.master_stores ms ON ft.store_id::TEXT = ms.store_id::TEXT
        WHERE ft.transaction_date >= CURRENT_DATE - INTERVAL '90 days'
        GROUP BY ms.city
    )
    SELECT
...
AS $$
    WITH behavior_data AS (
        SELECT
            ...
            ft.total_amount,
            ft.transaction_date,
            EXTRACT(hour FROM ft.transaction_time) as transaction_hour
        FROM scout.fact_transactions ft
        WHERE ft.transaction_date >= CURRENT_DATE - INTERVAL '90 days'
    )
    SELECT
...
-- =============================================================================
-- 6. ADDITIONAL DASHBOARD FUNCTIONS (simplified for compatibility)
-- =============================================================================

-- Product category performance (simplified)
...
AS $$
    WITH daily_data AS (
        SELECT
            ft.transaction_date as trend_date,
            COUNT(*) as transaction_count,
        ...
        GROUP BY ft.transaction_date, TO_CHAR(ft.transaction_date, 'Day')
    ),
    trend_analysis AS (
        SELECT
            *,
...
AS $$
    ...
    previous_period AS (
        ...
        FROM scout.fact_transactions ft
        WHERE ft.transaction_date >= CURRENT_DATE - INTERVAL '60 days'
        AND ft.transaction_date < CURRENT_DATE - INTERVAL '30 days'
    ),
    daily_revenue AS (
        SELECT
            ft.transaction_date as revenue_date,
...
-- ============================================================================
-- DEPLOYMENT COMPLETE - FIXED SCHEMA VERSION
-- Scout Dashboard Compatibility Package with 15+ Functions
